...
export default function Create() {
  ...
  const [date_start,setDateStart] = useState(Help.getDateInputFormat(Help.getUnixNum()));
  const [time_end,setTimeEnd] = useState("00:00");
  const [date_end,setDateEnd] = useState(Help.getDateInputFormat(Help.getUnixNum()));
  ...
  const submit =async ()=>{
    const level = easy?1:normal?2:hard?3:very_hard?4:0;
    ...
    if(!advantages.length){
      Toast.error("Phải có đánh giá thuận lợi !!");
      return;
    }
    if(!strengths.length){
      Toast.error("Phải có đánh giá ưu điểm !!");
      return;
    }
    if(!defects.length){
      Toast.error("Phải có đánh giá nhược điểm !!");
      return;
    }
    if(!level){
      Toast.error("Phải có đánh giá độ khó !!");
      return;
    }
    const data ={
      target:target,
    ...
    try{
      ...
      if(res.data.status==Contrast.SUCCESS)Toast.success(res.data.message);
